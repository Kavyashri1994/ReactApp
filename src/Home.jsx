...
class Home extends Component {
  ...
  onClickProducts() {
    let {showAllProducts} = this.state;
    this.props.history.push("Products");
    this.setState({showAllProducts: true})
  }

  render() {
    let {sarees, kurthis, bags, kids, showAllProducts} = this.state,
      items=[...sarees, ...kurthis, ...bags, ...kids],
    ...
    return (
      <div className="App">
        ...
        <ShoppingPage />
        <Products
          items={showAllProducts ? items:items.slice(0,4)}
          onClickProducts={this.onClickProducts.bind(this)}/>
        <BarItems />
        <Offers />
        <List items={[...sarees]} title={"Women"} />
        <List items={[...kurthis]} title={"Men"} />
        <BestSeller />
        <Message />
        <p className={"que"} style={{"font-size":"40px"}}> Have questions?</p>
        <p style={{"font-size":"20px"}} className={"mail-2"}>Email Us at <i className={"fa fa-envelope icon-mail"} style={{"font-size":"20px"}}/> care@myshopprime.com</p>
      </div>
    );
  }
}
